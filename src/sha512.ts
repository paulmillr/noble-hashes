...
export class _Sha512 extends Sha2 {
  ...
  _process(view: DataView, offset: number) {
    ...
    for (let i = 0; i < 16; i++, offset += 4) {
      SHA512_Wh[i] = view.getUint32(offset);
      SHA512_Wl[i] = view.getUint32((offset += 4));
    }
    for (let i = 16; i < 80; i++) {
      // s0 := (w[i-15] rightrotate 1) xor (w[i-15] rightrotate 8) xor (w[i-15] rightshift 7)
      const W15h = SHA512_Wh[i - 15] | 0;
      const W15l = SHA512_Wl[i - 15] | 0;
      const s0h = u64.rotrSH(W15h, W15l, 1) ^ u64.rotrSH(W15h, W15l, 8) ^ u64.shrSH(W15h, W15l, 7);
      const s0l = u64.rotrSL(W15h, W15l, 1) ^ u64.rotrSL(W15h, W15l, 8) ^ u64.shrSL(W15h, W15l, 7);
      // s1 := (w[i-2] rightrotate 19) xor (w[i-2] rightrotate 61) xor (w[i-2] rightshift 6)
      const W2h = SHA512_Wh[i - 2] | 0;
      const W2l = SHA512_Wl[i - 2] | 0;
      const s1h = u64.rotrSH(W2h, W2l, 19) ^ u64.rotrBH(W2h, W2l, 61) ^ u64.shrSH(W2h, W2l, 6);
      const s1l = u64.rotrSL(W2h, W2l, 19) ^ u64.rotrBL(W2h, W2l, 61) ^ u64.shrSL(W2h, W2l, 6);
      // SHA256_W[i] = s0 + s1 + SHA256_W[i - 7] + SHA256_W[i - 16];
      const SUMl = u64.add4L(s0l, s1l, SHA512_Wl[i - 7], SHA512_Wl[i - 16]);
      const SUMh = u64.add4H(SUMl, s0h, s1h, SHA512_Wh[i - 7], SHA512_Wh[i - 16]);
      SHA512_Wh[i] = SUMh | 0;
      SHA512_Wl[i] = SUMl | 0;
    }
    ...
    for (let i = 0; i < 80; i++) {
      ...
      // prettier-ignore
      const T1ll = u64.add5L(Hl, sigma1l, CHIl, SHA512_Kl[i], SHA512_Wl[i]);
      const T1h = u64.add5H(T1ll, Hh, sigma1h, CHIh, SHA512_Kh[i], SHA512_Wh[i]);
      const T1l = T1ll | 0;
