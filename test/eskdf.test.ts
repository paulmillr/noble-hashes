...
describe('eskdf', () => {
  should('vectors, types, protocol names', async () => {
    for (const v of vectors.derive_main_seed.valid) {
      ...
      try {
        // Main seed bytes are intentionally not exposed by eskdf().
    ...
    for (const v of vectors.derive_main_seed.invalid) {
    ...
    try {
      for (const v of vectors.derive_child_key.valid) {
    ...
    e = await eskdf(username, password);
    try {
